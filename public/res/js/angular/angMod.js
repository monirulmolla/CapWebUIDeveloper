...
angmodule.factory('CrudService', function($http, $q,http_url) {
    return {
        ...
        'get': function(reqUrl) {
            var defer = $q.defer();
            $http.get(http_url+reqUrl).success(function(resp){
                console.log("I am inside angMode Get:sucess:"+JSON.stringify(resp));
                defer.resolve(JSON.stringify(resp));
            }).error( function(err) {
                console.log("I am inside angMode Get:failure:"+JSON.stringify(err));
                defer.reject(err);
            });
            return defer.promise;
        },
        'create': function(reqUrl, data) {
            console.log("I am inside angMode:create function:reqUrl"+reqUrl);
            console.log("I am inside angMode:create function:Data:"+JSON.stringify(data));
            var defer = $q.defer();
            $http.post(http_url+reqUrl, data).success(function(resp){
                console.log("I am inside angMode:sucess:"+JSON.stringify(resp));
                defer.resolve(resp);
            }).error( function(err) {
                console.log("I am inside angMode:failure:"+JSON.stringify(err));
                defer.reject(err);
            });
            return defer.promise;
        },
        'update': function(reqUrl, data){
            console.log("I am inside angMode:update function:reqUrl"+reqUrl);
            console.log("I am inside angMode:update function:Data:"+JSON.stringify(data));
            var deferred = $q.defer();
            $http.put(http_url+reqUrl, data).success(function (response) {
                console.log("I am inside angMode:update sucess:"+JSON.stringify(response));
                deferred.resolve(response);
            }).error(function (err) {
                console.log("I am inside angMode: update failure:"+JSON.stringify(err));
                 deferred.reject(err)
            });
