...
function StudentCtrl($scope, $state, $stateParams, CrudService){
    ...
    if($stateParams.studentId != undefined){
        CrudService.get('/college/'+$stateParams.id+'/student/'+$stateParams.studentId).then(function(response){
            console.log("GetStudent: Step 3: angStudent.js");
            console.log("GetStudent: Step 3: student::"+response);
            var resp = JSON.parse(response)
            console.log("GetStudent: Step 3: studentItem::"+resp.student[0]);

            if(resp.code == 200){
                $scope.item = resp.student[0];
                $scope.item.rollNo = Number(resp.student[0].rollNo);
                $scope.item.yearOfJoining = Number(resp.student[0].yearOfJoining);
                $scope.item.hostel = resp.student[0].hostel._id;
            }
