...
function CountryListCtrl($scope, $state, CrudService, $timeout){
    ...
    var countryList = function(){
        CrudService.get('/countries').then(function(response){
            ...
            console.log("TEST:Response getCountry:::"+resp);
            if(resp.code == 200){
    ...
    $scope.addCountry = function(item){
        console.log("TEST: I am inside add country function");
        console.log("TEST: input data :::"+ JSON.stringify(item));
        console.log("TEST: input data id :::"+ item._id);
        if(item._id){
            console.log(item);
            CrudService.update('/country/'+item._id, item).then(function(response){
                console.log("TEST:Response updateCountry:::"+response);
            ...
            CrudService.create('/country', item).then(function(response){
                console.log("AddCountry:"+JSON.stringify(response));
                console.log(response);
                if(response.code ==200){
                    $scope.dismiss();
                    countryList();
...
function StateListCtrl($scope, $state, CrudService, $timeout){
    ...
    $scope.getCountryList = function(){
        CrudService.get('/activeCountries').then(function(response){
            console.log(response);
            var resp = JSON.parse(response);
            console.log("TEST:Response getActiveCountry:::"+resp);
            if(resp.code == 200){
                $scope.countries = resp.countries;
            }
        }, function(errorResponse){
    ...
    $scope.addState = function(item){
        if(item._id){
            ...
            CrudService.create('/state', item).then(function(response){
                console.log("AddState:"+JSON.stringify(response));
                console.log(response);
                $scope.dismiss();
                stateList();
